...
/**
 * @title Season Two Story staking contract
 * @dev This is currently identical to SeasonOne but some unnecessary logic
 * may be removed from SeasonOne (e.g. rollover).  So this is left as a
 * functional template we can work on going forward.
 */
contract SeasonTwo is Context, ISeason {
    ...
    modifier inClaimPeriod() {
        require(_isClaimPeriod(), 'SeasonTwo: Not claim period');
        _;
    }
    ...
    function wrapUp() external override ready {
        require(
            ...
            'SeasonTwo: Claim period not over'
        );
        ...
        if (ethBalance > 0) {
            ...
            require(success, 'SeasonTwo: Transfer failed');
        }
