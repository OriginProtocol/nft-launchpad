...
describe("Ingest Contracts", () => {
  ...
  it("Can collect Stable Coin after deploy", async () => {
    ...
    const { MockUSDT, master, ingestRegistry, ingestMaster, pool } = fixture
  ...
  it("Will fail on collect with no transfer", async () => {
    ...
    await expect( 
      ingestMaster.collect(salt, ETH, 123) 
  ...
  it("Can collect batch", async () => {
    ...
    const salts = [formatBytes32String("1"), formatBytes32String("2"), formatBytes32String("3"), formatBytes32String("4")]
    const amounts = [parseUnits('0.00000001', 18), parseUnits('0.00005', 18), parseUnits('1', 18), parseUnits('100000000', 18)]
    const tx = await _collect_batch(ingestMaster, MockUSDT, pool, salts, amounts)
  ...
  it("Can collect batch same salt", async () => {
    ...
    const tx = await _collect_batch(ingestMaster, MockUSDT, pool, salts, amounts)
    console.log("⛽️ Deploy and collect 2x same salt: gas used", await getGas(tx))
  })
  ...
  it("Cannot collect with bad args", async () => {
    ...
    const salts = [formatBytes32String("1"), formatBytes32String("2"), formatBytes32String("3"), formatBytes32String("4")]
    const coins = [MockUSDT.address, MockUSDT.address, MockUSDT.address, MockUSDT.address]
    const amounts = [parseUnits('0.00000001', 18), parseUnits('0.00005', 18), parseUnits('1', 18), parseUnits('100000000', 18)]
  ...
  it("Can change Collector", async () => {
    ...
    await ingestMaster.connect(newCollector).collect(salt, MockUSDT.address, amount)
  ...
  it("Can change registry", async () => {
    ...
    await ingestRegistry.setEndpointImplimentation(newImplAddress)
    expect(await ingestRegistry.endpointImplimentation()).to.be.equal(newImplAddress)
