...
describe('OriginERC721_v4Factory', () => {
  describe('Token with limited supply of 10', () => {
    ...
    it('Can create token with a minter', async () => {
      ...
      token = nftv4.attach(createTokenEvent.args.addr)
  ...
  describe('Payment splits', () => {
    ...
    before(async () => {
      const { factoryV4, master, deployer, nftv4 } = await loadFixture(
        defaultFixture
      )
...
async function getSigForMint({
  ...
  return await minter.signMessage(ethers.utils.arrayify(hash))
}
